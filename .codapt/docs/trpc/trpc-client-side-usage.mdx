...
## Important reminders about using tRPC with Tanstack React Query

When you want to pass normal Tanstack React Query options to a tRPC query (or mutation), do not pass them directly to useQuery or useMutation. Instead, pass them as the second argument to the `queryOptions` or `mutationOptions` functions.
